...
def test_create_objects_from_json(test_data):
    ...
    assert len(categories[0].__products) == 2
    assert isinstance(categories[0].__products[0], Product)
    assert categories[0].__products[0].name == "Product 1"
    assert categories[0].__products[0].price == 10.0
